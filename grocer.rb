def consolidate_cart(cart)
	  ...
	  cart.each do | grocery | 
	    grocery.each do | product, values | 
	      consolidated_cart[product] = { price: values[:price], clearance: values[:clearance], count: cart.count(grocery) }
	    end
	  end
	  consolidated_cart
	end
...
def apply_clearance(cart)
	  cart.each do 
	    |grocery,value| cart[grocery][:price] = (cart[grocery][:price] * 0.8).round(2) if cart[grocery][:clearance]
  end
end
